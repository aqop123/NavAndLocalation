...
def generate_launch_description():
    ...
    get_package_share_directory('bring_up'), 'config','measurement_params.yaml')))
    robot_description = Command(['xacro ', os.path.join(
    get_package_share_directory('bring_up'), 'urdf', 'sentry_robot_real.xacro'),
    ...
    segmentation_params = os.path.join(bringup_dir, 'config', 'segmentation.yaml')
    ...
    pointlio_mid360_params = os.path.join(bringup_dir, 'config', 'pointlio_mid360.yaml')
    ...
    icp_registration_params_dir = os.path.join(bringup_dir, 'config', 'icp_registration.yaml')
    ################################# icp_registration parameters end #################################

    small_gicp_relocalization_params_dir = os.path.join(bringup_dir, 'config', 'gicp.yaml')
    ...
    declare_world_cmd = DeclareLaunchArgument(
        'world',
        default_value='RIGHT_X',
        description='Select world (map file, pcd file, world file share the same name prefix as the this parameter)')
    
    declare_REL_cmd = DeclareLaunchArgument(
        'relocalation',
        default_value='icp',
        description='Choose relocalation alogrithm: icp or gicp')
    ...
    start_localization_group = GroupAction(
        actions=[
            TimerAction(
                condition = LaunchConfigurationEquals('relocalation', 'icp'),
                period=7.0,
                actions=[
                    Node(
            ...
            TimerAction(
                condition = LaunchConfigurationEquals('relocalation', 'gicp'),
                period=0.0,
                actions=[
                    Node(
                        ...
                        output='screen',
                        parameters=[
                            small_gicp_relocalization_params_dir,
                            {'use_sim_time': use_sim_time,
