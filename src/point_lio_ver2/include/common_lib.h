...
/***
 * @brief 时间序列 time_seq 的作用是记录点云数据中每个连续区间的长度，这些区间是根据点云数据中点的曲率变化来划分的。
 * 
*/
template<typename T>
std::vector<int> time_compressing(const PointCloudXYZI::Ptr &point_cloud)
{
  ...
  for(int i = 0; i < points_size - 1; i++)
  {
    ...
    {//代码通过比较相邻点的曲率值来检测曲率的变化。如果当前点的曲率大于下一个点的曲率，说明曲率发生了显著变化。
      time_seq.emplace_back(j); // 每当检测到曲率变化时，当前的计数器 j 的值会被记录到 time_seq 中，表示当前区间的长度
      j = 0;
    }
  }
//   if (j == 0)
