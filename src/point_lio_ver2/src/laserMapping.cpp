  ...
  std::unique_ptr<tf2_ros::Buffer> & tf_buffer, rclcpp::Logger logger_)
{
  ...
  transform.header.frame_id = "lidar_odom";
...
int main(int argc, char ** argv)
{
  ...
  /*-------------订阅livox/lidar的话题(mid360是订阅CustomMsg的话题)-----------*/
  ...
  while (rclcpp::ok()) {
    ...
    if (sync_packages(Measures)) {
      ...
      if (!use_imu_as_input) {
        ...
        effct_feat_num = 0;
        if (time_seq.size() > 0) {  //点云数据有不同曲率的数据（曲率变化点通常对应于点云数据中的重要特征点，时间序列可以帮助提取这些特征点的位置和分布）
          double pcl_beg_time = Measures.lidar_beg_time;  //雷达的开始时间
          idx = -1;
          ...
          for (k = 0; k < time_seq.size(); k++) {
            ...
            if (publish_odometry_without_downsample) {  //这个是有对点云降采样
              /******* Publish odometry *******/
            ...
            for (int j = 0; j < time_seq[k]; j++) {
              PointType & point_body_j = feats_down_body->points[idx + j + 1];  //通过时间序列找到特征点
              PointType & point_world_j = feats_down_world->points[idx + j + 1];
              pointBodyToWorld(&point_body_j, &point_world_j);
            }
            solve_time += omp_get_wtime() - solve_start;  //计算里程计发布和坐标转换时间

            update_time += omp_get_wtime() - t_update_start;  //计算点云更新时间
            idx = idx + time_seq[k];
          }
        } else {
          if (!imu_deque.empty()) {
      ...
      if (!publish_odometry_without_downsample) { //这个是没有对点云进行降采样
        publish_odometry(pubOdomAftMapped, tf_broadcaster, tf_buffer, nh->get_logger());
      }
      ...
      if (feats_down_size > 4) {
        MapIncremental(); //添加点云到地图中的函数
      }

      t5 = omp_get_wtime();
      /******* Publish points *******/
      if (path_en) publish_path(pubPath); //发布轨迹路径
      if (scan_pub_en || pcd_save_en) publish_frame_world(pubLaserCloudFullRes);  // 发布世界坐标系下的点云
      if (scan_pub_en && scan_body_pub_en) publish_frame_body(pubLaserCloudFullRes_body); // 发布本体坐标系下的点云
