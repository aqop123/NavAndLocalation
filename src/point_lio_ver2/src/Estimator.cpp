...
std::vector<PointVector> Nearest_Points; //最近的点云点
...
V3D Lidar_T_wrt_IMU(Zero3d);	//初始化IMU旋转矩阵
M3D Lidar_R_wrt_IMU(Eye3d);
double G_m_s2 = 9.81;	//重力系数
 ...
 * @brief 更新模型测量输入
 * 
 * 该函数根据当前状态和观测数据更新扩展卡尔曼滤波器的测量输入
 * 它处理特征点数据，计算有效测量的数量，并构建测量矩阵和向量
 * 
 * @param s 当前状态输入，包含机器人的状态信息
 * @param cov_p 位置协方差矩阵，表示位置测量的不确定性
 * @param cov_R 旋转协方差矩阵，表示旋转测量的不确定性
 * @param ekfom_data 扩展卡尔曼滤波器的数据结构，用于存储滤波过程中的中间结果和输出
 */
void h_model_input(state_input &s, Eigen::Matrix3d cov_p, Eigen::Matrix3d cov_R, esekfom::dyn_share_modified<double> &ekfom_data)
{
	...
	{
		// 获取当前特征点在车身坐标系和世界坐标系中的表示
		PointType &point_body_j  = feats_down_body->points[idx+j+1];
		PointType &point_world_j = feats_down_world->points[idx+j+1];

		pointBodyToWorld(&point_body_j, &point_world_j); // 将车身坐标系下的点转换到世界坐标系下
		V3D p_body = pbody_list[idx+j+1];	 // 获取当前特征点在车身坐标系下的位置向量
