...
IcpNode::IcpNode(const rclcpp::NodeOptions &options)
    : Node("icp_registration", options), first_scan_(true) {
  is_ready_ = false;
  cloud_in_ =
  ...
  icp_rough_.setMaxCorrespondenceDistance(0.10);
  icp_rough_.setInputTarget(rough_map_);

  icp_refine_.setMaximumIterations(refine_iter_);
  icp_refine_.setMaxCorrespondenceDistance(0.05);
  icp_refine_.setInputTarget(refine_map_);

  RCLCPP_INFO(this->get_logger(), "pcd point size: refine_map_: %ld, rough_map_: %ld",
              refine_map_->size(), rough_map_->size());

  // Parameters
  rough_iter_ = this->declare_parameter("rough_iter", 10);
  ...
  x_offset_ = this->declare_parameter("x_offset", 0.2);
  y_offset_ = this->declare_parameter("y_offset", 0.2);
  yaw_offset_ = this->declare_parameter("yaw_offset", 30.0) * M_PI / 180.0;
  yaw_resolution_ =
      this->declare_parameter("yaw_resolution", 10.0) * M_PI / 180.0;
  roll_offset_ = this->declare_parameter("roll_offset", 30.0) * M_PI / 180.0;
  roll_resolution_ = this->declare_parameter("roll_resolution", 10.0) * M_PI / 180.0;
...
void IcpNode::pointcloudCallback(
  ...
  // RCLCPP_INFO(this->get_logger(), "pointcloudCallback");
  if (first_scan_) {
    auto pose_msg =
...
void IcpNode::initialPoseCallback(
  ...
  Eigen::Matrix4d map_to_laser = multiAlignSync(cloud_in_, initial_guess);  // 获得雷达最有可能的位姿
...
Eigen::Matrix4d IcpNode::multiAlignSync(PointCloudXYZI::Ptr source,
  ...
  for (int i = -1; i <= 1; i++) {  // x 偏移
    for (int j = -1; j <= 1; j++) {  // y 偏移
      for (int k = -yaw_offset_; k <= yaw_offset_; k++) {  // yaw 角
        for (int l = -roll_offset_; l <= roll_offset_; l++) {  // roll 角
          Eigen::Vector3f pos(xyz(0) + i * x_offset_, xyz(1) + j * y_offset_,
                              xyz(2));
          Eigen::AngleAxisf rollAngle(rpy(0) + l * roll_resolution_,
  ...
  PointCloudXYZIN::Ptr rough_source_norm = addNorm(rough_source); // 添加法线
  PointCloudXYZIN::Ptr refine_source_norm = addNorm(refine_source);
  PointCloudXYZIN::Ptr align_point(new PointCloudXYZIN);

  Eigen::Matrix4f best_rough_transform; // 粗匹配的最佳匹配结果
  double best_rough_score = 1.0;
  bool rough_converge = false;
  auto tic = std::chrono::system_clock::now();

  for (Eigen::Matrix4f &init_pose : candidates) { // 遍历所有候选位姿
    icp_rough_.setInputSource(rough_source_norm);
    icp_rough_.align(*align_point, init_pose);  // 返回对齐匹配结果、align_point为匹配后输出的点云
    ...
    if (rough_score < best_rough_score) {
      ...
      rough_converge = true;
      best_rough_transform = icp_rough_.getFinalTransformation(); // 获得最佳匹配结果的变换矩阵
    }
  }
